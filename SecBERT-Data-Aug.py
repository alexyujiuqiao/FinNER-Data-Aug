...
from transformers import AutoTokenizer, TFAutoModelForTokenClassification, BertTokenizer, TFBertForMaskedLM, pipeline
...
def augment_sentence(tokens, labels):
    """Augment a single sentence with synonym replacement"""
    # Only try to replace words that are not entities (label 'O')
    replaceable_indices = [i for i, label in enumerate(labels) if label == 'O']
    
    if replaceable_indices and len(tokens) > 0:  # Add length check
        # Ensure indices don't exceed token length
        valid_indices = [idx for idx in replaceable_indices if idx < len(tokens)]
        
        if valid_indices:  # Only proceed if we have valid indices
            # Randomly choose how many words to replace (up to 30% of replaceable words)
            num_replacements = random.randint(1, max(1, int(len(valid_indices) * 0.3)))
            indices_to_replace = random.sample(valid_indices, min(num_replacements, len(valid_indices)))
            
            augmented_tokens = tokens.copy()
            for idx in indices_to_replace:
                word = tokens[idx]
                # Get POS tag to ensure replacement makes sense
                pos = pos_tag([word])[0][1]
                
                # Only replace if the word is a noun, verb, adjective, or adverb
                if pos.startswith(('NN', 'VB', 'JJ', 'RB')):
                    synonyms = get_synonyms(word)
                    if synonyms:
                        augmented_tokens[idx] = random.choice(synonyms)
            
            return augmented_tokens
    
    return tokens  # Return original tokens if no augmentation possible
...
def random_insertion(sentence, labels, p=0.1):
    ...
    for token, label in zip(sentence, labels):
        ...
        if random.random() < p:
            ...
            new_labels.append("O")  # Neutral label
...
def noisy_labels(labels, p=0.05):
    ...
    return [random.choice(label_list) if random.random() < p else lbl for lbl in labels]

# Initialize BERT for masked language modeling
bert_tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
bert_mlm = TFBertForMaskedLM.from_pretrained('bert-base-uncased')
...
def augment_dataset(texts, labels, num_augmentations=1):
    augmented_texts = []
    augmented_labels = []
    
    print(f"Original training size: {len(texts)}")
    ...
    for text, label_seq in zip(texts, labels):
        ...
        tokens = tokenizer.convert_ids_to_tokens(text['input_ids'][0])
        original_labels = [reverse_label_map[l] for l in label_seq]
        ...
        if random.random() < 0.5:  # Contextual replacement
            aug_tokens, aug_labels = contextual_replacement(valid_tokens, original_labels)
            augmented_text = tokenizer(' '.join(aug_tokens), padding='max_length',
                                     truncation=True, max_length=128, return_tensors='tf')
            augmented_texts.append(augmented_text)
            augmented_labels.append(label_seq)
        
        if random.random() < 0.3:  # Synonym replacement
            aug_tokens, aug_labels = synonym_replacement(valid_tokens, original_labels)
            augmented_text = tokenizer(' '.join(aug_tokens), padding='max_length',
                                     truncation=True, max_length=128, return_tensors='tf')
            augmented_texts.append(augmented_text)
            augmented_labels.append(label_seq)
        
        if random.random() < 0.2:  # Token deletion
            aug_tokens, aug_labels = token_deletion(valid_tokens, original_labels)
            augmented_text = tokenizer(' '.join(aug_tokens), padding='max_length',
                                     truncation=True, max_length=128, return_tensors='tf')
            augmented_texts.append(augmented_text)
            augmented_labels.append(label_seq)
        
        if random.random() < 0.3:  # Random insertion
            aug_tokens, aug_labels = random_insertion(valid_tokens, original_labels)
            augmented_text = tokenizer(' '.join(aug_tokens), padding='max_length',
                                     truncation=True, max_length=128, return_tensors='tf')
            augmented_texts.append(augmented_text)
            augmented_labels.append(label_seq)
        
        if random.random() < 0.2:  # Noisy labels
            noisy_label_seq = noisy_labels(original_labels)
            augmented_texts.append(text)
            augmented_labels.append([label_map[l] for l in noisy_label_seq])
...
train_texts, train_labels = augment_dataset(train_texts, train_labels, num_augmentations=2)
